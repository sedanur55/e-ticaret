...
class ProductController extends AbstractController
{
     ...
     */
    public function iedit(Request $request, Product $product, $id): Response  //Resimler üzerinde düzenleme
    {
        $form = $this->createForm(ProductType::class, $product);
        $form->handleRequest($request);
     ...
     */
    public function iupdate(Request $request, Product $product): Response   //Fotoğraf upload
    {
        $form = $this->createForm(ProductType::class, $product);
        $form->handleRequest($request);
        $file=$request->files->get('imagename');
        $fileName=$this->generateUniqueFileName().'.'.$file->guessExtension(); //file dosyasını generateUniqueFileName ile adını değiştiriyoruz.
        try {
            $file->move(
                $this->getParameter('image_directory'),   //config/services.yaml da tanımlı resimin yolu
                $fileName
            );
